...
import { Clock, Star, CheckCircle2, ChevronRight, ArrowRight } from 'lucide-react'
...
function DaysUntil({ dueDate }) {
  ...
  return <span className="text-xs text-slate-500">{diff}d left</span>
}
...
function UpcomingCard({ mission }) {
  ...
  return (
    <div className="group relative rounded-2xl border border-scout-border bg-scout-card p-4 transition-all duration-200 hover:border-scout-muted hover:shadow-xl hover:shadow-black/30">
      {/* Faded overlay to signal "locked" state */}
      ...
      <div className="relative space-y-3">
        ...
        <div>
          <h3 className="text-sm font-semibold text-slate-300 group-hover:text-white">{mission.title}</h3>
          <p className="mt-1 text-xs leading-relaxed text-slate-500">{mission.teaser}</p>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-600">Opens {formatDate(mission.dueDate)}</span>
        </div>
      </div>
    </div>
  )
    ...
    >
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <p className="mt-1 text-xs leading-relaxed text-slate-400">{mission.teaser}</p>
        </div>
  ...
  return (
    ...
    >
      <div className="space-y-3">
        ...
        <div>
          <h3 className="text-sm font-semibold text-slate-400 line-through decoration-slate-600">{mission.title}</h3>
          <p className="mt-1 text-xs leading-relaxed text-slate-600">{mission.teaser}</p>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          {submittedDate && (
            <span className="text-xs text-slate-600">Submitted {submittedDate}</span>
          )}
        </div>
      </div>
    </button>
